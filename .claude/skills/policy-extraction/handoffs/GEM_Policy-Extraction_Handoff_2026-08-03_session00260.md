...
| `gem_audit.py` | `3ae8d6c4299c098162f16162ae3be0c4` | 314462 | **M** |
...
32. **`gem-policy-docx` runtime path unverified (S260).** `python-docx` is not installed on this machine, so the ported script was verified **statically only** — syntax parses, `--files-dir` default is `.`, output default is cwd, no `/mnt/` paths remain. It has never been executed here. Resolve with `pip install python-docx` and one export run before relying on it.
...
34. **`.claude/settings.json` sets `PYTHONUTF8=1`.** This masked the L214 defect and would mask its class again. Now that L214 is explicit, consider whether the env var should stay — it is a machine-level crutch that makes the scripts appear correct on a machine that has it and fail on one that does not. Pending Tom.

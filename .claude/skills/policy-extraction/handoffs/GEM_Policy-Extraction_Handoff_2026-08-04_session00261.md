...
39. **Promote-queue ratio is invisible in the worklist (S260).** Now **709 `planPromote` stubs against 164 `planDone`**. `policy_worklist.json` holds 22 entries and no longer reflects the backlog.
40. **`memory edit #13` still reads as a followable pointer (S260).** Cosmetic; gloss it in place or drop the clause.
41. **The NCD corpus has no denominator — this gates "all NCDs are processed".** The graph holds **149** NCD sections, each either extracted or cited by something extracted, and nothing has ever compared that set against the list CMS publishes. **S261 is the third consecutive demonstration**: NCD 50.2, like NCD 20.15 and NCD 190.4 before it, was absent from the graph entirely and reached the corpus only because Tom named it. Resolution needs an authoritative list — an MCD export or the Pub. 100-03 table of contents from Tom, or Tom's go-ahead to fetch and reconcile the CMS NCD index. **Raise this before declaring the NCD priority met.**

...
|-------|--------|-------------------------|--------|
| `CIM` | Coverage Issues Manual (HCFA Pub. 6) | `"6"` | TN 1–174, into 2003 |
...
**The era gate — a disqualifier that outranks every source below.** Pub. 100-03, 100-04 and 100-20 did not exist before 2003, and the CIM ended at TN 174 in 2003. Therefore a transmittal dated before 2003 **cannot** carry `NCD`/`CP`/`OTN`, and one numbered above 174 **cannot** carry `CIM`. (Boundary corrected 167→168 at S177, then 168→169 at S247: TN 169's rendition R169CIM.pdf — the 04/2003 crystallization-boundary transmittal that removed the age limitation on stem-cell-transplantation coverage for multiple myeloma, NCD 110.23 — was supplied by Tom, so the CIM reached TN 169 and just into 2003; the number half of the gate is authoritative, the date half is advisory. **Corrected again 169→171 at S267**, when NCD 220.6's promotion produced a RED on `gemi:tn171CIM` and the source overruled the ceiling: Version 2's own Coverage Transmittal Link publishes `R171CIM.pdf`, and Tom supplied the same URL. Its 06/2003 revision-history entry carries effective and implementation dates of 10/01/2003, so the CIM ran past 'early 2003' by a further quarter. **Then corrected a second time in the same session, 171→174**, when Tom located R172CIM, R173CIM and R174CIM — and, having searched for R175CIM and **not** found it, gave the ceiling its first bound from **above**. Every previous raise had pushed the number up from below on the one rendition that happened to be needed; this is the first time anyone asked where the manual actually stops. That makes 174 a materially better number than its four predecessors, though still a negative-search result rather than a documented terminus — a rendition for 175 would overturn it, and nothing here proves one does not exist. **The ceiling is an empirical high-water mark, not a documented fact** — five raises (167, 168 at S177, 169 at S247, 171 and 174 at S267), every one driven by a rendition arriving rather than by reasoning, and each time the guard fired correctly on a number it had not yet seen. Read a `transmittal_manual_token` RED on a CIM token as a question about the ceiling first and about the individual second, and resolve it only on a rendition. **TN 172, 173 and 174 are deliberately NOT minted**: no policy in the corpus cites them, and the S131 rule mints what is referenced, not what is known to exist. They enter the graph with their renditions at the first citation.)

---
description: Close the GEM session — run the SKILL.md close checklist, then commit and push.
---

Close the current GEM session.

**`SKILL.md` §Session Close is the authority for every step below.** This command
is the invocation and the commit; it deliberately does not restate the checklist,
because two copies of a procedure is the drift class this project exists to fight.
Read §Session Close and follow it as written.

Order is load-bearing. Do not reorder:

1. **Fetch and reconcile with `origin` FIRST.**

       git fetch && git status -sb

   If `main` is behind or diverged, reconcile now (`git pull --rebase`) before
   any other step. Doing this later is too late: a remote change to a canonical
   file would rewrite bytes that step 6's §1 table has already hashed, and the
   table would stop describing the tree being pushed. `git status` alone cannot
   detect this — without a fetch it compares against a possibly-stale local
   `origin/main` ref and reports agreement with a pointer rather than with the
   remote. That is exactly what happened at S260.

2. Confirm every in-session edit is written to the canonical directory.
3. If any policy's HCPCS code references or any `gem:CodeGroup` membership changed,
   re-run the `gem:refersToCodeGroup` linking pass. `codegroup_link_drift` and
   `codegroup_block_extent` must both be GREEN before proceeding.
4. **Write the new handoff first**, so its `Session N` title is on disk before the
   audit runs. This is what makes the `empirical_counts` marker target N instead of
   N−1 (the Fix-A protocol, S237).
5. Run the audit with `--autofix`. Confirm GREEN, and confirm the empirical-counts
   marker in `SKILL.md` reads **S{N}**, not S{N−1}. Any later rewrite of `SKILL.md`
   reopens the lag and requires re-running this step.
6. **Compute the §1 table LAST.** Hash and size all 15 canonical files only after
   every one is in its final state, then write the table into the new handoff. Any
   canonical file touched after the table is computed silently invalidates its row
   and yields a spurious `hash_verify` RED at the next bootstrap.
7. Commit:

       git add -A
       git commit -m "S{N}: <one-line summary>"

   The handoff and the source PDFs are part of the commit.

8. Push:

       git push

   Only after step 5 is GREEN and step 6's table is written — a push before
   that publishes a §1 table that does not describe the pushed bytes. The
   remote (`github.com/tjkelly3va/GEM`, private) is the only off-machine copy
   of `sources/`, so do not stop at the commit.

9. Report: what changed, the audit tier counts, the INFO queue totals, the NCD
   census line, and the commit SHA.

If the audit is not GREEN at step 5, stop and surface the finding. Do not compute
the §1 table over an unclean set, and do not commit.
